...
class StroySpider(scrapy.Spider):
    ...
    def parse(self, response):
        def text_to_string(list):
            ...
            # r = re.findall(p, s)
            return ''.join(list)
        section = response.css('div.content h1::text').get()
        textinfo = response.css('div.textinfo span::text').getall()
        content = text_to_string(response.css('div.showtxt::text').getall())
